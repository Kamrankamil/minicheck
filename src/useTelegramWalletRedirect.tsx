...
/**
 * Runtime initializer — converts deep links / WalletConnect URIs into universal links
 * and uses Telegram.WebApp.openLink when inside Telegram mobile WebView.
 */
  ...
  try {
    ...
    const tryOpen = (url?: string | URL | null) => {
      ...
      try {
        ...
        // ✅ Extract WalletConnect URI if present
        ...
        // ✅ If WalletConnect URI found, build universal links
        if (wcUri) {
          ...
          console.log("✅ WalletConnect URI detected, using MetaMask universal link");
        ...
        // ✅ Open in Telegram WebApp if inside Telegram
        if (isTG) {
        ...
        console.error("❌ Wallet link open failed, falling back to window.open:", err);
        try {
...
/**
 * React hook version (optional - for component-level usage)
 */
